...
import { FileText, Clock, ExternalLink, Search, X } from "lucide-react"
import { DashboardSkeleton } from "@/components/ui/skeletons"
import { formatDistanceToNow, format } from "date-fns"
import { Link } from "react-router-dom"

export default function TDMonitoringPage() {
  const [samples, setSamples] = useState<Sample[]>([])
  const [loading, setLoading] = useState(true)
  const [error, setError] = useState<string | null>(null)

  // Filter states for Current Work tab
  ...
  return (
    <div className="space-y-6 p-6 bg-background">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          <h1 className="text-2xl font-semibold tracking-tight text-foreground">TD Monitoring</h1>
          <p className="text-sm text-muted-foreground">Track samples at PSI stage and upcoming submissions.</p>
        </div>
      </div>
      ...
      <div className="grid gap-4 md:grid-cols-2">
        <Card>
          ...
          <CardContent>
            ...
            <p className="text-xs text-muted-foreground mt-1">Samples at PSI stage</p>
          </CardContent>
        </Card>
      ...
      <Tabs defaultValue="current" className="w-full">
        ...
        <TabsContent value="current" className="mt-6">
          <Card>
            <CardHeader>
              <CardTitle>Current PSI Work</CardTitle>
              <CardDescription>
                Samples currently at PSI stage
        ...
        <TabsContent value="upcoming" className="mt-6">
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                New sample submissions that will reach PSI
                    ...
                    <TableBody>
                      {filteredSamples.upcoming.map((sample) => (
                        <TableRow key={sample.id}>
                          ...
                          <TableCell>{sample.brands?.name || "—"}</TableCell>
                          <TableCell>
                            <Badge variant="outline">{sample.current_status || "New"}</Badge>
                          </TableCell>
                          <TableCell>
                            <div className="flex flex-col">
                              <span className="text-sm">{format(new Date(sample.created_at), "MMM dd, yyyy")}</span>
                              <span className="text-xs text-muted-foreground">
