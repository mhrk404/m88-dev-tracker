...
import { ClipboardCheck, Clock, ExternalLink, Search, X } from "lucide-react"
...
export default function MDMonitoringPage() {
  ...
  const categorizedSamples = useMemo(() => {
    ...
    // Current - samples at PC_REVIEW stage
    const current = activeSamples.filter((sample) => {
      const stage = String(sample.current_stage || "").trim().toUpperCase()
      return stage === "PC_REVIEW"
    })

    // Upcoming - samples at SAMPLE_DEVELOPMENT stage (will come to review next)
  ...
  return (
    <div className="space-y-6 p-6 bg-background">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          <h1 className="text-2xl font-semibold tracking-tight text-foreground">MD Monitoring</h1>
          <p className="text-sm text-muted-foreground">Track samples for PC review and upcoming evaluations.</p>
        </div>
      ...
      <div className="grid gap-4 md:grid-cols-2">
        <Card>
          ...
          <CardContent>
            ...
            <p className="text-xs text-muted-foreground mt-1">Samples awaiting review</p>
          </CardContent>
        </Card>
      ...
      <Tabs defaultValue="current" className="w-full">
        <TabsList className="grid w-full grid-cols-2">
          <TabsTrigger value="current" className="gap-2">
            ...
            {categorizedSamples.current.length > 0 && (
              <Badge variant="secondary" className="ml-1 bg-green-100 text-green-700 dark:bg-green-900 dark:text-green-100">
        ...
        <TabsContent value="current" className="mt-6">
          <Card>
            <CardHeader>
              <CardTitle>For PC Review</CardTitle>
              <CardDescription>
                Samples ready for PC review evaluation
        ...
        <TabsContent value="upcoming" className="mt-6">
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                Samples in development that will need review
