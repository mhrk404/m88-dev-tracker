...
export const create = async (req, res) => {
  try {
    ...
    await logAudit({ userId: req.user?.id, action: 'create', resource: 'product_category', resourceId: String(data?.id), details: { name: data?.name }, ip, userAgent });
...
export const remove = async (req, res) => {
  try {
    ...
    if (error) {
      if (error.code === '23503') return res.status(409).json({ error: 'Product category is in use and cannot be deleted' });
      throw error;
    }
    const { ip, userAgent } = auditMeta(req);
