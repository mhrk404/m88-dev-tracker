...
import {
  SAMPLE_ROLE_KEYS,
  setSampleRoleOwner,
} from "../services/sampleRoleOwnersService.js";
...
function toAuditStage(stageLike) {
  const normalized = String(stageLike || "")
    .trim()
    .toLowerCase();
  switch (normalized) {
    case "psi":
...
async function getSampleAssignment(sampleId) {
  const { data, error } = await supabase
    ...
    .select(
      "pbd_user_id, td_user_id, fty_user_id, fty_md2_user_id, md_user_id, costing_user_id",
    )
...
function resolveRoleOwnerForStage(stage, payload, user, assignment) {
  ...
  if (stage === "sample_development") {
    return {
      ...
      userId:
        payload?.fty_md_user_id ??
        (roleCode === "FTY"
          ? user?.id
          : (assignment?.fty_md2_user_id ?? assignment?.fty_user_id ?? null)),
    };
  }
  ...
  if (stage === "costing") {
    return {
      ...
      userId:
        payload?.team_member_user_id ??
        (roleCode === "COSTING"
          ? user?.id
          : (assignment?.costing_user_id ?? null)),
    };
  }
...
export const getStages = async (req, res) => {
  try {
    ...
    return res
      .status(500)
      .json({ error: err.message ?? "Failed to get stages" });
  }
};
...
export const updateStage = async (req, res) => {
  try {
    ...
    const conflictingLock = await findConflictingSampleLock({
      sampleId,
      userId: req.user?.id,
    });
    if (conflictingLock) {
      const blockerName =
        conflictingLock.full_name ||
        conflictingLock.username ||
        `User #${conflictingLock.user_id}`;
    ...
    if (!stageContext) {
      return res
        ...
        .json({
          error:
            "body.stage is required and must be one of: " +
            [...STAGE_TABLES, "delivered_confirmation"].join(", "),
        });
    }
    ...
    if (allowedStages != null && !allowedStages.includes(permissionStage)) {
      return res
        ...
        .json({
          error:
            "You can only update your own stage: " + allowedStages.join(" or "),
        });
    }
    ...
    if (existing) {
      console.log(
        "[DEBUG] Updating existing record with payload:",
      ...
      console.log(
        "[DEBUG] Inserting new record with payload:",
        JSON.stringify({ sample_id: sampleId, ...safePayload }, null, 2),
      );
      const { data: inserted, error } = await supabase
        .from(table)
        .insert({ sample_id: sampleId, ...safePayload })
        .select("*")
        .single();
      if (error) throw error;
      data = inserted;
    ...
    // Fetch assignment for all users to enable proper role owner tracking
    const assignment = await getSampleAssignment(sampleId);
    const roleOwner = resolveRoleOwnerForStage(
      requestedStage,
      safePayload,
      req.user,
      assignment,
    ...
    if (
      roleOwner?.roleKey &&
      roleOwner.userId != null &&
      !Number.isNaN(Number(roleOwner.userId))
    ) {
    ...
    if (advance_to_stage) {
      ...
      if (!expectedNext || requestedNext !== expectedNext) {
        return res
          .status(400)
          .json({
            error: `Invalid stage transition. Allowed next stage from ${requestedStage} is ${expectedNext ?? "none"}.`,
          });
      }
    ...
    const canceledInPayload =
      isCanceledLike(safePayload.current_status) ||
      isCanceledLike(safePayload.sample_status) ||
      isCanceledLike(safePayload.status);

    const hasDeliveredSentDate =
      requestedStage === "delivered_confirmation" &&
    ...
    if (canceledInPayload) {
      ...
      if (!hasDeliveredSentDate) {
        return res
          .status(400)
          .json({
            error: "sent_date is required to mark sample as delivered.",
          });
      }
    ...
    const { error: statusErr } = await supabase
      .from("sample_request")
      .update({
        current_status: mappedCurrentStatus,
        sample_status: mappedSampleStatus,
      })
      .eq("sample_id", sampleId);
    if (statusErr)
      console.error("Failed to apply sample/stage status mapping:", statusErr);
    ...
    return res
      .status(500)
      .json({ error: err.message ?? "Failed to update stage" });
  }
};
